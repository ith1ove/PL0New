...
void MainWindow::GetSym() {
    ...
    if ((CH>='A' && CH<='Z') || (CH>='a' && CH<='z')) { /*ID OR RESERVED WORD*/
        K=0;
        do {
            if (K<AL) A[K++]=toupper(CH);  // 转换为大写存储
            GetCh();
    ...
    while (SymIn(SYM,FACBEGSYS)) {
        ...
        if (SYM==PLUSPLUS || SYM==MINUSMINUS) {
            ...
            if (SYM==IDENT) {
                i=POSITION(ID,TX);
                if (i==0) Error(11);
                else if (TABLE[i].KIND!=VARIABLE && TABLE[i].KIND!=CHARVARIABLE) Error(12);
                else {
                    // 前缀++: 先增加变量值，然后返回新值
                    // 加载变量当前值
            ...
            case 21: // 字符输入
                {
                    T++;
                    bool ok;
                    QString input = QInputDialog::getText(this, tr("字符输入"), tr("请输入一个字符："), QLineEdit::Normal, "", &ok);
                    if(ok && !input.isEmpty()){
